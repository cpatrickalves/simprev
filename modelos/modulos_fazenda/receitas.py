...
def calc_receitas(salarios, dadosLDO, periodo):

    # Dicionário que armazenas os resultados
    resultados = {}
    # Obtem as aliquotas efetivas médias
    aliquota = dadosLDO['AliqEfMed']
    ...
    for ano in periodo:
        for sexo in ['H','M']:
            for clientela in ['Ca', 'Csm']:            
                ...
                mSalContrTotal[ano] += salarios[id_sal][ano].sum() * fatAnualizacao
...
def calc_pib(resultados, salarios, PIBs, periodo):
